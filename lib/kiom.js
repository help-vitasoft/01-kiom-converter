...
export default {
    ...
    ann : {},
    image : "",
    a: "C:\\file\\contents\\volume-contents.yml",
    b: "C:\\file\\extractions\\meaning-classification.yml",
    exTagName: "",
    init(){
      this.src=null,
      this.LV="";
    ...
    changeImage(str){
      ...
      arr.push(str.replace(pattern,"$1"));
      arr.push(str.replace(pattern,"$2"));
      arr.push(str.replace(pattern,"$3"));
    	return arr;
    },
    changeNewLine(str){
    ...
    setAcupoint(arrayKey,keywordArray){
      if(!this.other['acupoint']){
        ...
        if(keywordArray[0].length>0)arrKWD["general"]["acupoint"]["acupoint_keyword"].push("\""+keywordArray[0]+"\"");
      ...
      if(!this.other['cure']){
        ...
        if(keywordArray){
          if(keywordArray[1].trim()){
      		    ...
      		    if(keywordArray[0].length>0)arrKWD['cure'][key1][keyword].push("\""+keywordArray[0]+"\"");
      	  }
        }
